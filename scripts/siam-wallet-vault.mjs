...
export const DEFAULT_SIAM_WALLET_VAULT = resolve(".autotrade/siam-wallets-vault.json");
export const DEFAULT_SIAM_WALLET_ADDRESSES = resolve(".autotrade/siam-wallet-addresses.json");
export const SIAM_WALLET_KEYCHAIN_SERVICE = "AutoTrade Siam wallet vault";
...
function storeVaultPassword(vaultPath, password) {
  ...
  const existing = runKeychain(
    ["find-generic-password", "-a", account, "-s", SIAM_WALLET_KEYCHAIN_SERVICE],
    { allowFailure: true },
  );
...
export async function writeEncryptedSiamWalletBackup({ vaultPath, addressesPath, secret, publicManifest }) {
  ...
  const vault = {
    format: "autotrade-siam-wallet-vault",
    version: 1,
    cipher: "aes-256-gcm",
    kdf: "scrypt",
    keychain: {
      service: SIAM_WALLET_KEYCHAIN_SERVICE,
...
export async function decryptSiamWalletBackup(vaultPath = DEFAULT_SIAM_WALLET_VAULT) {
  const resolvedVaultPath = resolve(vaultPath);
  const vault = JSON.parse(await readFile(resolvedVaultPath, "utf8"));
  const expectedAccount = keychainAccount(resolvedVaultPath);
  if (
    vault.format !== "autotrade-siam-wallet-vault" ||
    vault.version !== 1 ||
    vault.cipher !== "aes-256-gcm" ||
    vault.kdf !== "scrypt" ||
    vault.keychain?.service !== SIAM_WALLET_KEYCHAIN_SERVICE ||
    vault.keychain?.account !== expectedAccount
  ) {
    throw new Error("Unsupported or misplaced Siam wallet vault.");
  }
