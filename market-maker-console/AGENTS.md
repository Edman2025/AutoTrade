...
- On 2026-09-11 the operator changed the monitored asset to 暹罗币 (`EFQcTNWXTtjMBQfHt7R5EQFpq8X2agwKeA43DfVrkgan`). Its freshly verified Meteora DAMM v2 pool is `EJhr4va8YcksrUhDAMrTH6yeZUBZKzen2KYEaiLJNFPe`; the approved topology is now `暹罗币/ANTFUN ↔ ANTFUN/USDT`. SOL is a native fee reserve only and must not appear as a quote, route, valuation, or market-making asset. Do not select alternative pool addresses without fresh identity/liquidity verification.
...
- The Emit-inspired capability set is deliberately AMM-safe: fixed-route batch quotes, external-signing execution queues, current priority-fee telemetry, explicit Jito/Nozomi configuration state, and monitored-token Mint/holder intelligence. Do not add browser-held signing keys, fake atomicity, alternative DEX routes, wash-volume automation, holder inflation, or price-manipulation controls.
- Operator-facing growth labels must preserve those semantics: “成交增长” means verified pool-volume analytics; “持币地址” means every positive Token Account aggregated by owner; “盈利地址” is explicitly limited to the locally indexed analysis window; “实时交易” is parsed from confirmed main-pool vault deltas; and “流动性变化” comes from persisted mainnet snapshots. Buy/sell inventory execution remains risk-gated fixed-route slicing without price targets. Renaming must never conceal wash trading, fake-holder creation, pumping, or dumping.
- Staking operations record tables use client-side pagination with a fixed 20 records per page; each record tab keeps an independent page position.
